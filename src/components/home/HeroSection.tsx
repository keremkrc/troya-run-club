...
export default async function HeroSection() {
  ...
  return (
    <div className="relative min-h-[640px] md:min-h-[720px] overflow-hidden bg-[#0F172A] flex items-center justify-center">
      ...
      {/* ── Overlay: koyu siyah, opacity 0.55 ── */}
      <div className="absolute inset-0 z-10 bg-black/55" />

      {/* ── Alt vignet (yazıların zemine karışmaması için) ── */}
      <div className="absolute inset-0 z-10 bg-gradient-to-t from-[#0E0E0E]/80 via-transparent to-transparent" />

      {/* ── İçerik ── */}
      <div className="relative z-20 px-6 md:px-12 pb-16 pt-32 max-w-4xl w-full text-center mx-auto">
        ...
        <p
          className="text-[15px] font-light text-cream/80 mt-5 leading-[1.7] max-w-xl mx-auto animate-fade-up opacity-0"
        ...
        >
          <Link
            href="/kayit"
            className="bg-bronze-500 hover:bg-bronze-600 text-white text-[11px] font-semibold tracking-[2px] uppercase px-8 py-3.5 transition-colors"
          >
            Gruba Katıl
          </Link>
          <Link
            href="/#antrenmanlar"
            className="text-cream text-[11px] font-semibold tracking-[2px] uppercase px-0 py-3.5 border-b border-cream/50 hover:text-bronze-400 transition-colors"
          >
            Programı Gör →
          </Link>
        </div>
